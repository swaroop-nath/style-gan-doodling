...
class EqualizedConv2dLayer(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=3, padding=1, padding_mode='zeros', stride=1, **kwargs):
        ...
        self.conv = nn.Conv2d(in_channels=in_ch, out_channels=out_ch, kernel_size=kernel_size, padding=padding, padding_mode=padding_mode, stride=stride)
        self.conv.bias = None
...
class StyleMod(nn.Module):
    def __init__(self, latent_dim, channel_size):
        self.affine_stds = EqualizedLinearLayer(latent_dim, channel_size ** 2)
        self.affine_biases = EqualizedLinearLayer(latent_dim, channel_size ** 2)
